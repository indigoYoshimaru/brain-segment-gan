...
from optimization.losses import dice_loss_indiv, dice_loss_mix
...
for epoch in tqdm(range(start_epoch, max_epoch), ncols=70):
    ...
    for de in tqdm(range(dis_epoch)): 
        ...
        for idx, sampled_batch in enumerate(trainloader):
            ...
            pred_fake = dis_net(volume_batch, softmax_pred)
            loss_fake = mse_loss_func(pred_fake, fake_gt)
            loss_d = loss_real + loss_fake
            epoch_dis_loss+=loss_d

            dis_iter_num+=1
            logging.info(f'Discriminator {idx}/{de}/{epoch}: loss real: {loss_real.item()} -- loss fake: {loss_fake}')
            loss_d.backward()
            dis_opt.step()
    ...
    for idx, sampled_batch in enumerate(trainloader):
        ...
        # Adversarial grounde truths, real = 1, fake = 0
        valid_gt = Tensor(np.ones((volume_batch.size(0),1,14,14,12)))
        # fake_gt =Tensor(np.zeros((volume_batch.size(0), 1,8,8,8)))
        softmax_pred, fake_mask = gen_net(volume_batch)
        ...
        loss_g = total_ce_loss + total_dice_loss + loss_GAN
        gen_iter_num += 1 
        gen_opt.zero_grad()
        loss_g.backward()
        gen_opt.step()
        ...
        logging.info(f'Generator {idx}/{epoch}: \n   Cross-entropy loss : {total_ce_loss}\n   Dice loss: {total_dice_loss}\n   L2 loss: {loss_GAN}')
        logging.info(f'Class dice loss: {dice_losses}')
        writer.add_scalar('generator-sample/cross entropy loss',
                                  total_ce_loss.item(), gen_iter_num)
        ...
        if gen_iter_num % 50 ==0:
            draw_image(volume_batch,'train/Image', from_slice=0, to_slice=1, iter_num=gen_iter_num, coords = params_cfg['coords']) 
            draw_image(softmax_pred,'train/Softmax_label', from_slice=1, to_slice=2, iter_num=gen_iter_num, coords = params_cfg['coords']) 
            draw_image(outputs_soft,'train/Predicted_label', from_slice=1, to_slice=2, iter_num=gen_iter_num, coords = params_cfg['coords']) 
            draw_image(mask_batch,'train/Groundtruth_label', from_slice=1, to_slice=2, iter_num=gen_iter_num, coords = params_cfg['coords']) 

            # image = volume_batch[0, 0:1, :, :, 20:61:10].permute(
            #     3, 0, 1, 2).repeat(1, 3, 1, 1)

            # grid_image = make_grid(image, 5, normalize=True)
            # writer.add_image('train/Image', grid_image, gen_iter_num)

            # image = softmax_pred[0, 1:2, :, :, 20:61:10].permute(
            #     3, 0, 1, 2).repeat(1, 3, 1, 1)
            # grid_image = make_grid(image, 5, normalize=False)
            # writer.add_image('train/Softmax_label', grid_image, gen_iter_num)

            # image = outputs_soft[0, 1:2, :, :, 20:61:10].permute(
            #     3, 0, 1, 2).repeat(1, 3, 1, 1)
            # grid_image = make_grid(image, 5, normalize=False)
            # writer.add_image('train/Predicted_label', grid_image, gen_iter_num)

            # image = mask_batch[0, 1:2, :, :, 20:61:10].permute(
            #     3, 0, 1, 2).repeat(1, 3, 1, 1)
            # grid_image = make_grid(image, 5, normalize=False)
            # writer.add_image('train/Groundtruth_label',
            #                     grid_image, gen_iter_num)
